        ...
        for ($i = 0; $i < count($galleriesList); $i++) {
            ...
            if ($gallery->getIsPublished()) {
                $json_arr[($gallery->getGalleryType()->getId() == 1)?'photo':'video'][] = array(
                    ...
                    'previewImage' => ($gallery->getGalleryType()->getId() == 1)?$gallery->getMainPic()->getFrontendPicture():"./img/play.jpeg",
                    'url' => ($gallery->getGalleryType()->getId() == 1)?'#/gallery/photo/':'#/gallery/video/'.$gallery->getId()
